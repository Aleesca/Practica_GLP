...
| `Proyecto/Especificaciones/autonomia.md` | `Memoria` + `Calculos justificados` | `RESUMEN DE CARACTERÍSTICAS.` y `CONSUMO Y AUTONOMÍA.` | La capacidad final alimenta el resumen; el desarrollo va al capítulo de Cálculos justificados. |
...
| `Proyecto/Especificaciones/metodologia-longitud-calculo.md` | `Memoria` + `Calculos justificados` | `CARACTERÍSTICAS DE LOS EQUIPOS. -> Canalizaciones.` y desarrollo de tuberías en `Cálculos justificados` | En Memoria se integra el diagrama del método iterativo adaptado de Mermaid a LaTeX y la explicación metodológica; en Cálculos se desarrolla la comprobación numérica y la tabla de resultados. |
...
| `Proyecto/Especificaciones/anejos.md` | `Calculos justificados` (3.1) + `Planos` (capítulo) | `ÍNDICE DE ANEXOS.` y `Planos` (Capítulo sin subsecciones) | Delimita anexos reales y planos; no crea un anejo independiente de cálculos. |
| `Proyecto/Especificaciones/calculos-justificados.md` | `Calculos justificados` | Capítulo completo `Cálculos justificados` | Contrato editorial del capítulo: estructura, tabla A4 horizontal de tuberías y bibliografía propia. |
...
- Integrar en `Memoria`, dentro de red de distribución/canalizaciones, el diagrama del método iterativo de dimensionado hidráulico adaptado de Mermaid a LaTeX (`tikzpicture` o figura vectorial/PDF), con caption, label y referencia desde el texto.
- Llevar a `Cálculos justificados` el desarrollo numérico, tablas de comprobación y resultados intermedios.
- La tabla de dimensionado de tuberías debe insertarse como hoja aislada A4 horizontal dentro de `Cálculos justificados`, conservando encabezado y pie de página.
- La bibliografía de `Cálculos justificados` debe cerrar el capítulo con el mismo criterio funcional que la bibliografía de Memoria.
